...
function Sports() {
  ...
  const sportsProducts = products.filter(
    (product) => product.product_category?.toLowerCase() === "sports"
  );

  const indexOfLast = currentPage * productsPerPage;
  const indexOfFirst = indexOfLast - productsPerPage;
  const currentProducts = sportsProducts.slice(indexOfFirst, indexOfLast);
  const totalPages = Math.ceil(sportsProducts.length / productsPerPage);
  ...
  return (
    <div className="container-fluid p-5 products-container">
      <div className="row g-4">
        <h2 className="text-center">Amazon Sports</h2>
